...
- **Backend over-exposure (flagged, not fixed):** `GET /api/nodes/{id}` returns node **earnings to any authenticated caller** — the UI gates them to the owner, but the endpoint doesn't. Server-side hardening, out of frontend scope but worth a backend pass.
