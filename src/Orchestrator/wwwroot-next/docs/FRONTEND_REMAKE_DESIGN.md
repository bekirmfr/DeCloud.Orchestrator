...
- **⚠ Billing/payments — NOT YET DESIGNED, added 2026-07-25.** This category was missing from both the route tree above and this inventory, and it is the one genuine design gap found when auditing which legacy surfaces have replacements. What exists in `/app` today is **read-only**: balance, burn rate, runway (`useBalance.ts`, polled), plus the hard fund gate on deploy. Every **write** is still legacy-only, in `payment.js`: the on-chain escrow **deposit** (ethers → `escrow.deposit`, reached via the sidebar balance card → balance modal → `openDepositFromBalance()`), **earnings withdrawal** (`withdrawFromBalanceModal`), and admin **platform-fee withdrawal** (`withdrawPlatformFees`, gated by the escrow contract's own `onlyOwner`). `DEPLOY_MIGRATION.md` already tracks the deposit bridge as debt with an open action; withdrawals are tracked nowhere. **No route is assigned here deliberately — that is an owner decision**, and the natural candidates differ in kind: a `/app/billing` surface of its own, a modal-route off `/app/settings`, or keeping deposit as a bridge to the legacy modal until Phase 6 and only porting withdrawals. **Whichever is chosen, the constraint is the same: the legacy app cannot be deleted while it is the only place money moves.**
