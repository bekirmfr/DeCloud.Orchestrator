...
Grounding the serving layer turned up a **surface undercount** — **resolved in DESIGN v0.8 as a decision. ⚠ NOT YET BUILT (verified 2026-07-25).**

> **Read this before executing Change 3.** "Resolved" below means *the design decided where these go*, not *the code exists*. As of 2026-07-25 `wwwroot-next/src/features/` contains only `billing`, `dashboard`, `deploy`, `ssh-keys`, `vms` — **no terminal, no files** — there is no `xterm` dependency in `wwwroot-next/package.json`, and `VmDetailPage.tsx` links to neither. The standalone pages and their WebSocket proxies are still the only working implementation. **Dropping them from the Change 3 standalone list before the in-app routes ship would take two live surfaces offline.** Worse, they are already unreachable from the UI — the `window.open` calls that opened them live in `vm-modals.js:133,140`, whose callers went with the retired VM table, so users have had no way in since 2026-07-24 (tracked in `FRONTEND_REMAKE_IMPLEMENTATION.md` §8 alongside the direct-access/custom-domains regression). **Change 3 gate: keep serving `terminal.html` and `file-browser.html` until `/app/vms/:id/terminal` and `/app/vms/:id/files` are built and reachable.**

- **`terminal.html` + `file-browser.html` → in-app routes** (`/app/vms/:id/terminal`, `/app/vms/:id/files`) with pop-out preserved. They were `window.open('_blank')` pop-out pages; plain xterm/WS with no COOP/COEP, so no serving-isolation concern. **Serving consequence once built: two fewer standalone entries.**
- **`tos.html`, `sign.html`, `report.html` stay standalone.**
- **Updated Change 3 topology (conditional on the above being built):** the standalone-entries list to keep serving after the `/`-flip becomes **`sign.html`, `report.html`, `tos.html`** — but **only once the two in-app routes exist**; until then it remains all five. Everything else in §3 unchanged.
