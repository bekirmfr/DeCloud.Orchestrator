...
| **5** | Supporting paths (marketplace, my-templates, nodes, settings, admin) | ◐ In progress — **Marketplace shipped**: browse + detail + Markdown descriptions, a deploy-source chooser (empty / marketplace / GitHub), and a working GitHub repo-deploy form. The stale `template-detail.js` pricing tables are retired and the shell's Deploy button is unhardcoded off `platform-general` (see 2026-07-26 journal). Remaining: template authoring (my-templates + admin review), nodes browse, settings | Owner | 2026-07-26 | — |
