...
- **`npm ci` in `BuildFrontendNext` is gated on `node_modules` being absent**, which is a proxy for the thing that matters (does the tree match the lockfile) rather than the thing itself — so a stale-but-present `node_modules` silently skips installing a newly added dependency, and the build fails later with a `TS2307` that looks unrelated. Currently mitigated by a *manual ritual* documented in `AGENT_HANDOUT.md` §8, which is the wrong kind of fix. **Planned:** a `RestoreFrontendNext` target with `Inputs="package-lock.json"` / `Outputs="node_modules/.package-lock.json"` (npm's own post-install artifact — verified present on the production server, with an mtime later than the lockfile's), letting MSBuild track the real dependency. Measured cost of the naive alternative (dropping the condition so `npm ci` always runs): **47.8s per build**, so the incremental target is worth having rather than just always installing. The legacy `BuildFrontend` target runs `npm ci` unconditionally today and would get the same treatment — a net speedup.
- **Source maps ship to production.** `vite.config.ts` sets `sourcemap: true`, so `/app/assets/*.map` is served, including a ~6.3 MB map for the main chunk — publishing full readable source and comments. Not a breach (client code is inspectable regardless), but a default rather than a decision. `sourcemap: false`, or `hidden` to keep maps as build artifacts without the browser link.
