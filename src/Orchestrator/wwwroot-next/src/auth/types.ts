...
export interface AuthUser {
  ...
  roles: string[]; // server User.Roles, e.g. ["User"] or ["User","Admin"]
  email?: string | null;
...
/** The chain the app transacts on. Set `VITE_EXPECTED_CHAIN_ID` per environment
 *  to match the backend's `Payment:ChainId` (Polygon mainnet = 137, Amoy testnet
 *  = 80002). Falls back to 137. The pure `deriveStatus` takes this as a parameter,
 *  so tests are unaffected; this constant only sets the runtime default. */
export const EXPECTED_CHAIN_ID = Number(import.meta.env.VITE_EXPECTED_CHAIN_ID) || 137;