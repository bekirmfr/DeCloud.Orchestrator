...
import {
  ...
  ARTIFACT_TYPES, CHECK_STRATEGIES, VISIBILITIES, PRICING_MODELS,
} from "./templateForm";
...
const label = (pairs: [number, string][], v?: number) =>
  pairs.find(([n]) => n === v)?.[1] ?? (v == null ? "—" : String(v));
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: "var(--text-sm)" }}>
          {t.variables.map((v, i) => (
            <div key={i} style={{ color: "var(--text-secondary)" }}>
              <code style={{ fontFamily: "var(--font-mono)" }}>{v.name}</code> · {label(VARIABLE_KINDS, typeof v.kind === "number" ? v.kind : undefined)}
              {typeof v.kind === "number" && v.kind === 1 ? ` · ${label(WATCHER_SCOPES, typeof v.scope === "number" ? v.scope : undefined)}` : v.required ? " · required" : " · optional"}
