...
function Help({ text }: { text: string }) {
    ...
    return (
        <span style={{ position: "relative", display: "inline-flex" }}
            ...
            {open && (
                <span role="tooltip" style={{ position: "absolute", bottom: "calc(100% + 6px)", left: 0, zIndex: 20, width: "min(250px, calc(100vw - 24px))", padding: "8px 10px", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", color: "var(--text-secondary)", fontSize: "var(--text-xs)", lineHeight: 1.45, boxShadow: "0 8px 24px rgba(0,0,0,0.4)", fontWeight: 400 }}>{text}</span>
            )}
        </span>
    );
}
            ...
            <div style={sectionStyle}>
                ...
                {form.envVars.map((e, i) => (
                    <div key={i} style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <input style={{ flex: "1 1 120px" }} placeholder="KEY" value={e.key} onChange={(ev) => set("envVars", form.envVars.map((r, j) => j === i ? { ...r, key: ev.target.value } : r))} />
                        <input style={{ flex: "2 1 160px" }} placeholder="value" value={e.value} onChange={(ev) => set("envVars", form.envVars.map((r, j) => j === i ? { ...r, value: ev.target.value } : r))} />
            ...
            <div style={sectionStyle}>
                ...
                {form.artifacts.map((a, i) => (
                    <div key={i} style={{ display: "flex", flexDirection: "column", gap: 6, padding: "8px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-sm)" }}>
                        ...
                        {a.mode === "inlineText" && (<>
                            <input style={{ width: 240, maxWidth: "100%" }} placeholder="content type (e.g. text/x-sh)" value={a.contentType} onChange={(ev) => set("artifacts", updRow(form.artifacts, i, { contentType: ev.target.value }))} />
