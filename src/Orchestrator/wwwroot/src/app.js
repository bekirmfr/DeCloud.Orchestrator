...
async function loadFeaturedNodes() {
    ...
    try {
        const response = await fetch(`${CONFIG.orchestratorUrl}/api/marketplace/nodes/featured`);
        const data = await response.json();
...
async function searchMarketplace() {
    ...
    try {
        const response = await fetch(`${CONFIG.orchestratorUrl}/api/marketplace/nodes?${params.toString()}`);
        const data = await response.json();
...
function renderNodeCards(container, nodes) {
    ...
    container.innerHTML = nodes.map(node => {
        ...
        const tagsHtml = (node.tags || []).map(tag => {
            ...
            return `<span class="mp-tag ${tagClass}">${escapeHtml(tag)}</span>`;
        }).join('');

        // GPU badge
        const gpuBadge = caps.hasGpu
            ? `<span class="mp-tag gpu">${escapeHtml(caps.gpuModel || 'GPU')}${caps.gpuCount > 1 ? ' x' + caps.gpuCount : ''}</span>`
            : '';
        ...
        return `
            <div class="mp-node-card" onclick="openNodeDetail('${escapeHtml(node.nodeId)}')">
                <div class="mp-card-header">
                    <div>
                        <div class="mp-node-name">${escapeHtml(node.operatorName || node.nodeId.substring(0, 12))}</div>
                        <div class="mp-node-region">${escapeHtml(node.region || 'Unknown')}${node.zone ? ' / ' + escapeHtml(node.zone) : ''}</div>
                    </div>
...
async function openNodeDetail(nodeId) {
    ...
    try {
        const response = await fetch(`${CONFIG.orchestratorUrl}/api/marketplace/nodes/${encodeURIComponent(nodeId)}`);
        const data = await response.json();
        ...
        if (data.success && data.data) {
            ...
            title.textContent = escapeHtml(node.operatorName || 'Node Details');
            ...
            body.innerHTML = `
                ${node.description ? `<div class="node-detail-section full-width"><div class="node-detail-desc">${escapeHtml(node.description)}</div></div>` : ''}
                ...
                <div class="node-detail-grid">
                    <div class="node-detail-section">
                        ...
                        <div class="node-detail-row">
                            ...
                            <span class="node-detail-value">${escapeHtml(caps.cpuModel || 'Unknown')} (${caps.cpuCores || 0} cores)</span>
                        </div>
                        <div class="node-detail-row">
                        ...
                        <div class="node-detail-row">
                            ...
                            <span class="node-detail-value">${escapeHtml(caps.gpuModel || 'Yes')}${caps.gpuCount > 1 ? ' x' + caps.gpuCount : ''}</span>
                        </div>` : ''}
                        <div class="node-detail-row">
                    ...
                    <div class="node-detail-section">
                        ...
                        <div class="node-detail-row">
                            ...
                            <span class="node-detail-value">${escapeHtml(node.region || 'Unknown')}</span>
                        </div>
                        <div class="node-detail-row">
                            <span class="node-detail-label">Zone</span>
                            <span class="node-detail-value">${escapeHtml(node.zone || 'N/A')}</span>
                        </div>
                        <div class="node-detail-row">
                            <span class="node-detail-label">Node ID</span>
                            <span class="node-detail-value" style="font-size: 11px;">${escapeHtml(node.nodeId)}</span>
                        </div>
                    </div>
                </div>
