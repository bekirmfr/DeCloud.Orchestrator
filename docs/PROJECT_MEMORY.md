...
10. **Instrumentation is load-bearing** — during one investigation an unqualified `uptimeSeconds` in an agent-shaped payload was read as agent uptime when it was the host's, `strings` silently missed UTF-16 .NET literals (use `strings -el`), and journald discarded the only window that mattered. The bug took hours; the untrustworthy tooling took longer. A field whose name does not state its subject is a defect with the same weight as the code it describes — and note the first of these was itself misdiagnosed for most of the session as "reports the wrong value", when the value was always right and only the name was wrong.
