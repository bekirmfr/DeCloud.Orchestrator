...
**Slice 1b — edit-after-approval re-review.** A non-admin edit that changes the **deployable payload** (`CloudInitTemplate`/`ContainerImage`/`ExposedPorts`/`Variables`/`DefaultEnvironmentVariables`) of a template staying `Published` resets it to `PendingReview` and clears the review stamps — runs after the slice-1 clamp so `PendingReview` isn't reverted. `DeployableSignature()` excludes cosmetic fields and `Artifacts` (already preserved). Closes the swap-after-approve vector.
