...
RTX 4060    RTX 3070      RTX 4060Ti     RTX 3090
GPU Proxy ↓ GPU Proxy ↓  GPU Proxy ↓   GPU Proxy ↓
Host GPU    Host GPU      Host GPU       Host GPU
```

### 3.1 LLM Layer VM

An optional system VM type. A node agent with a CUDA-capable GPU and a
public IP may elect to run an LLM Layer VM, committing a configurable
fraction of its GPU VRAM to host a contiguous range of transformer layers
...
- Registers its availability in the DHT: `{vmId, model_cid, layer_start, layer_end, endpoint, load, trust_score}`
...
DHT Provider Records (per layer range):
  ...
  Value: { endpoint, vram_committed, current_sessions,
           tokens_served, trust_score, zone, region }
```
...
gpu_pool:
  # Fraction of GPU VRAM to commit to LLM layer hosting.
...
participation target for a mature zone.

### 4.2 Operator Comfort Guarantees

At 25% VRAM commitment:
...
| Existing primitive | How it is used (unchanged) |
|---|---|
| `gpu-proxy-daemon` | Each LLM Layer VM's RPC backend runs against it. No changes needed. |
...
| DHT provider records | Layer graph registry. Each LLM VM announces its record. Same mechanism as block CID announcements. |
| WireGuard mesh | Secure inter-stage activation transport. Already connects all nodes. |
| `NodeLocality` (region/zone) | Hard constraint on route assembly. `region` must match across all route members. |
| `IObligationEligibility` | Extended with `CheckLlm()` — GPU present, public IP, VRAM ≥ minimum per-stage size. |
...
- `CheckLlm()` eligibility predicate (GPU + public IP + VRAM threshold)
- `system-llm-layer` cloud-init template
  ...
  - Registers DHT provider record on Ready
...
LLM Layer VM without manual intervention.
...
| CGNAT layer hosts (no public IP) | Activation routing through 3-hop DNAT introduces unpredictable latency. May be reconsidered for same-LAN CGNAT nodes in a future zone design. |
...
2. **Layer range assignment on VRAM contention.** If an operator reduces
   ...
   changes take effect on next restart. Simple and predictable.
