...
**Active phase:** Phase 2 (2 of 6 tasks remaining — P2.5 verification, P2.3+P2.6 deferred bundle).

**Last session:** 2026-05-03 — P2.4 drafted, then split into A + B during testing-prep audit. **Discovery:** `OrchestratorClient.BaseUrl` was unset in all three orchestrator config sources (`appsettings.json`, `appsettings.Production.json`, `install.sh` env file). Without this, P2.4-B's renderer throws on every tenant deploy via `OrchestratorUrlResolver`. **P2.4-A** plumbs the config (mirroring node-agent's existing key of the same name). **P2.4-B** is the actual code cutover (constructor injection + STEP 6.5 branch). **Apply order: A before B**, otherwise first General deploy after restart throws. The systemd `EnvironmentFile=` plumbing is already in place (verified in install.sh:1489) — once P2.4-A's env-file line lands, the value is loaded automatically on orchestrator restart. **First task that runs the new pipeline in a live tenant deploy** — Phase 2's central design goal achieved.

**Next task:** P2.5 — verify `EnsureQemuGuestAgent` + GPU-shim safety nets still run when `CloudInitUserData` comes from the new pipeline. These are post-substitution concerns in `LibvirtVmManager.CreateCloudInitIso`; both work on the final string regardless of how it was produced. Verification is a smoke test, not a code change. After P2.5, P2.6 (marketplace template smoke test) closes Phase 2 — and any tenant resolvers needed by real marketplace templates surface as a P2.3 follow-up.
