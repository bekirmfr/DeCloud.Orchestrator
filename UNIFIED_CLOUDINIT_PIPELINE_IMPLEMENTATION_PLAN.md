...
**Active phase:** Phase 2 (P2.4 + P2.5 done; P2.3 deferred; P2.5-verify + P2.6 remain). Phase 3 unblocked once P2.6 closes.

**Last session:** 2026-05-04 — closed three deploy fixes (DEPLOY-FIX, BLOCKSTORE-FIX, P2.5 unified node deploy pipeline) and surfaced + half-fixed a CRLF line-ending bug in tenant artifacts. The architectural reframe of P2.5 (from "verification smoke test" to "extract `IVmDeploymentPipeline`") came out of a route-diagram analysis showing every VM type takes a different path through the deploy stack. Originally `TENANT-PREFETCH-FIX` was drafted to add prefetch directly inside `CommandProcessorService.HandleCreateVmAsync`, mirroring `SystemVmReconciler.ActCreateAsync`. That would have entrenched two parallel prefetch sites. The boundary correction: extract a single `IVmDeploymentPipeline` (prefetch + binary verify + manager dispatch) called from both. Patch landed and verified in repo. Tenant General VM `bu4-2770` deployed via the new pipeline — decloud-agent and attestation are working, terminal + file browser work — but `welcome.service` crashes on `env: 'python3\r'` because `tenant-vms/general/assets/general-api.py` was committed with CRLF endings. Pipeline preserves bytes faithfully; the defect is at the source.

**Next task:** Close the CRLF arc. User has normalized both affected files (`general-api.py`, `index.html`) to LF in `DeCloud.Builds`, regenerated `artifact-constants.cs` via `compute-artifact-constants.sh`. Remaining steps: paste new constants into `GeneralVmTemplateSeeder.cs`, bump `GeneralTemplateRevision` 1→2, build, restart orchestrator, delete `bu4-2770`, redeploy. This **also** satisfies the original P2.5 acceptance criterion (verify `EnsureQemuGuestAgent` + GPU-shim safety nets still run when `CloudInitUserData` comes from the new pipeline) — same redeploy demonstrates both. After that: P2.6 (marketplace smoke test) closes Phase 2.

**Pre-Phase-3 user action items (still open):**
- **Cut a `binaries/v1.1.0` release** for the updated BlockStore binary (P0.9 binary change committed but not yet released). Existing `v1.0.0` is in production and working — soft TODO, blocks Phase 3 BlockStore cutover only. Update the recorded SHA256 in §1 below when cut.

**Open small follow-ups:**
- **Cosmetic:** remove unused `_artifactCache` from `SystemVmReconciler` (field + ctor param + assignment, three lines). The pipeline owns prefetch now; field is dead weight.
- **SHA verification:** upstream `decloud-agent-amd64` shows `content-length: 5087384`, but `DecloudAgentAmd64Bytes` in `GeneralVmTemplateSeeder.cs` declares `5_087_232`. Run `curl -sL <url> | sha256sum` and compare against `DecloudAgentAmd64Sha256`. If different, refresh both constants and bump `GeneralTemplateRevision` again.
- **Orchestrator-side `Artifacts` payload addition** (P2.5 file 6) shipped in the same commit as the pipeline. CreateVm command now carries `Artifacts = template?.Artifacts ?? []`.
...
- [x] **P2.5** — *[REFRAMED 2026-05-04: extract `IVmDeploymentPipeline` + verify safety nets in same redeploy.]* Unified node-side deploy pipeline + EnsureQemuGuestAgent / GPU-shim verification.
  ...
  - **Smoke-test verification (pending — closes with bu4-2770 redeploy after CRLF fix):** Per design §5 Phase 2.5, confirm `EnsureQemuGuestAgent` + GPU-shim safety nets still run when `CloudInitUserData` comes from the new pipeline. These are post-substitution concerns in `LibvirtVmManager.CreateCloudInitIso`; verify they're still triggered when `CloudInitUserData` comes from the new pipeline. Acceptance: deploy a VM with no `qemu-guest-agent` declared in template — verify it gets injected. Deploy a Proxied-GPU VM — verify the shim is injected. **Both checks are exercised by the bu4-2770 redeploy** that closes the CRLF arc; no separate test scaffolding required.
  - **Cosmetic follow-up:** `_artifactCache` field in `SystemVmReconciler` is now unused (dead weight after the refactor). Remove field + ctor parameter + assignment in a separate three-line commit. Not blocking.
  ...
  - **Done 2026-05-04 (architectural extraction)** — patch applied and repo-verified. Smoke-test verification closes with the next tenant redeploy after the CRLF fix.
