...
**Active phase:** Phase 3 (relay → DHT → BlockStore cutover, one role at a time). Phase 2 functionally complete; all P2.x done or deferred per spec.

**Last session:** 2026-05-04 — closed the CRLF arc end-to-end on a fresh testbed reset, applied the P2.5 cosmetic cleanup (`_artifactCache` removal), surfaced + fixed a `__VM_NAME__`-in-artifact regression, and verified P2.6 marketplace smoke test. Sequence: full platform reset → all three system VMs (relay/DHT/BlockStore) deployed cleanly via the new pipeline → fresh General tenant VM deployed cleanly with welcome page reachable on `*.vms.stackfi.tech` ingress (shebang resolved, service running) → marketplace template (web-proxy-browser) deployed cleanly → noticed `__VM_NAME__` rendering as literal text in the welcome page HTML. Diagnosis: the artifact pipeline preserves bytes faithfully (same property that caused the CRLF bug). `__VM_NAME__` lives inside `index.html`'s bytes, shipped as a base64 `data:` URI; the orchestrator's `CloudInitRenderer` substitutes placeholders in the cloud-init YAML body, but artifact bytes are never traversed (per content-addressable design — substituting per-VM would defeat the SHA256 cache). Pre-P0.1 the welcome HTML was inlined as a `write_files` entry, so `LibvirtVmManager.cloudInitYaml.Replace("__VM_NAME__", spec.Name)` happened to substitute it; P0.1 moved welcome content to artifacts and severed that path. Boundary fix applied this session — substitution now happens on the VM, after artifact fetch (see §2 entry for boundary analysis). Decloud-agent SHA verification on `decloud-agent-amd64` confirmed `530c33c34…` matches the declared constant; the 152-byte `DecloudAgentAmd64Bytes` discrepancy is informational metadata only (SHA256 is the integrity gate everywhere — `ArtifactCacheService.VerifyAsync`, `TemplateService.VerifyExternalArtifactAsync`).

**Next task:** Phase 3 prep + P3.1.1 (relay seeder update). Phase 3 cuts over system VMs role-by-role; relay first because DHT and BlockStore depend on a working relay. Prep step blocks P3.3 only — see action items below.

**Pre-Phase-3 user action items (still open):**
- **Cut a `binaries/v1.1.0` release** for the updated BlockStore binary (P0.9 binary change committed in `DeCloud.Builds` but not yet released). Existing `v1.0.0` BlockStore binary is in production and working with the v6.0/v4.1/v8.1 cloud-init triplet. Soft TODO — blocks **Phase 3 BlockStore cutover (P3.3) only**; relay (P3.1) and DHT (P3.2) can proceed against `v1.0.0`. Manual GitHub Actions workflow dispatch on `binaries/v1.1.0` tag. Update the recorded SHA256 in this section when cut.
