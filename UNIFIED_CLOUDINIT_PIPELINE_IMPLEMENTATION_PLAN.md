...
**Active phase:** Phase 3, sub-phase 3.1 (relay cutover). P3.1.1–P3.1.5 done. P3.1.6 acceptance testing in progress (platform reset pending).

**Last session:** 2026-05-04 (continuing) — landed all five P3.1 implementation tasks. P3.1.1+P3.1.2: expanded relay seeder Variables list from 1 entry (`WIREGUARD_PUBLIC_KEY`) to 21 entries (all `__VARNAME__` placeholders in the v6.1 composed relay cloud-init) and added six relay-specific resolvers to `SystemVmResolvers`. P3.1.3: added `GET /api/obligations/{role}/environment` endpoint to NodeAgent (`ObligationEnvironmentController` + `ObligationEnvironment` model); relay returns empty `values`/`scopes` and deterministic generation `44136fa355ba77b9` (SHA256 of `{}`); node-side resolver infrastructure deferred to P3.2.3 (DHT has the first real Dynamic consumers). P3.1.4: relay cloud-init v6.1→v6.2 — added `/etc/decloud-relay/` env dir + scope file (`__VARIABLE_SCOPES_BLOCK__`) + stub environment file, added `EnvironmentFile=-/etc/decloud-relay/environment` to relay-api + relay-http-proxy service units, added watcher artifact install + initial env-fetch + `decloud-env-watcher@relay.timer` enable to runcmd; seeder amended to add `VARIABLE_SCOPES_BLOCK` to `BuildRelayVariables()` (missed in P3.1.1, caught here) and three watcher artifacts to relay artifact list. P3.1.5: `ActCreateAsync` becomes pure executor — `CloudInitUserData = template.CloudInitContent` (no more `SubstituteArtifactVariables` call), relay-specific label injection block removed (orchestrator renderer now handles `__RELAY_SUBNET__` via `RelaySubnetResolver`), labels reduced to `node-arch`/`system-vm-role`/`system-vm-revision`; `Guid.NewGuid()` kept (VmId inversion deferred to Phase 3 cleanup per BLOCKSTORE-FIX §6); stray patch instructions removed from comment block.

**Next task:** P3.1.6 — platform reset + relay acceptance testing (see checklist in §7.1).
